...
public class OrderServiceImpl implements OrderService {
    ...
    public int makerOrder(Order order) {
        //throw new RuntimeException("系统维护中，暂时无法下单");
        Map part = new HashMap();
        part.put("member_id",order.getMember_id());
        ...
        Member member = memberServiceImpl.findMemberById(order.getMember_id());
        int admin_id = member.getInvite_id();
        if(order.getTrade_direction() == 1){//如果是买并且第一次买这只股票必须选择钱多的券商账户
            ...
            if(os == null){//说明第一次买这个股票，指定代理就使用代理账户否则 就选择钱多的账

                List<broker> allValid = brokerService.getAllValid();
                broker broker = null;
                for(int i =0;i<allValid.size();i++){
                    if(allValid.get(i).getAdmin_id() == admin_id){

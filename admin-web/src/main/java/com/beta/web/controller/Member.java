...
public class Member {
    ...
    public RetResult getAll(@RequestParam Map<String ,Object> map)
    {
       ...
       List dd  = MemberServiceImpl.findCollections(map);
       double sum_depo = dd.stream().mapToDouble(s->{
           MemberHeYueApply m = (MemberHeYueApply)s;
           return m.getDeposit();
       }).sum();
        double sum_cap = dd.stream().mapToDouble(s->{
            MemberHeYueApply m = (MemberHeYueApply)s;
            return m.getTotal_capital();
        }).sum();
        double sum_lev = dd.stream().mapToDouble(s->{
            MemberHeYueApply m = (MemberHeYueApply)s;
            return m.getLeverage_money();
        }).sum();
        double sum_mar = dd.stream().mapToDouble(s->{
            MemberHeYueApply m = (MemberHeYueApply)s;
            List<nettyOrder> activeOrderMoney = m.getOrder_list();
            ...
            BigDecimal mv = new BigDecimal(m.getTotal_capital()).add(new BigDecimal(l)).setScale(2,RoundingMode.HALF_UP);
            return mv.doubleValue();
        }).sum();
        double profit = sum_mar - sum_depo-sum_lev;
       ...
       data.put("deposit",sum_depo);
       data.put("lev",sum_lev);
       data.put("cap",sum_mar);
       return RetResponse.makeOKRsp(data);
    }

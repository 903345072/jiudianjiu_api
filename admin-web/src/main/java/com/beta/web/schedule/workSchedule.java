...
//                        memberHeYueApply.increaseHeYueMoney(m1);
        ...
        if(order.getPid() != 0 && p_order.getStock_status() != 1 ){//加仓或减仓
            ...
            if(order.getTrade_direction() == 1){ //如果是买
                ...
                sum_order_money = p_order_money.add(chengjiao_money).add(get_sx_fee(new BigDecimal(real_hand),buy_price,userInfo.getSx_rate()));
                ...
                sum_order_money = p_order_money.subtract(chengjiao_money).add(get_yh_fee(new BigDecimal(real_hand),buy_price)).add(get_sx_fee(new BigDecimal(real_hand),buy_price,userInfo.getSx_rate()));
                ...
                BigDecimal back_money = chengjiao_money.subtract(get_yh_fee(new BigDecimal(real_hand),buy_price)).subtract(get_sx_fee(new BigDecimal(real_hand),buy_price,userInfo.getSx_rate()));
    ...
    public void xuYue(){
        ...
        canEffectHeYue.forEach(s->{
            memberService.renewHeYue(s.getId(),1);
            memberService.xuyue(s.getId());//再放回队列
        });
    }
